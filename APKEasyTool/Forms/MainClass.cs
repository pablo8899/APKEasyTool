...
namespace APKEasyTool
{
    public class MainClass
    {
        ...
        private string pathToHistory = Variables.TempPath;
        ...
        internal async void Init()
        {
            ...
            var namesOnly = filePaths.Select(f => Path.GetFileName(f)).OrderByDescending(x => x).ToArray();
            ...
            await Task.Factory.StartNew(() =>
            {
                if (chkForUpdate == true && Environment.GetCommandLineArgs().Length < 2 && NetworkInterface.GetIsNetworkAvailable() == true)
                {
                    ...
                    string easyToolVersion = Updates.EasyToolVersion();
                    if (easyToolVersion != "ERROR")
                    {
                        if (easyToolVersion != Program.Version)
                            main.LogOutput(Lang.Localize("update_lbl", "A new version is available:") + " " + easyToolVersion, MainForm.Type.Info);
                        else
                            main.LogOutput(Lang.NO_UPDATE, MainForm.Type.Info);
                    }

                    ApkToolUpdate apktoolUpdate = Updates.ApkToolVersion();
                    string apktoolName = apktoolUpdate.Values[0]?.Name ?? "";
                    List<string> localApktoolVersions = LocalToolsVersion.LocalApkToolVersion();
                    if (!localApktoolVersions.Any(x => x.Contains(apktoolName)))
                    {
                        main.LogOutput(Lang.Localize("update_apktool_lbl", "A new apktool version is available, starting downloading file :") + " " + apktoolName, MainForm.Type.Info);
                        new WebClient().DownloadFile(apktoolUpdate.Values[0].DownloadUrl, "ApkTool/" + apktoolName);


                        filePaths = Directory.GetFiles(Variables.RealPath("Apktool"), "*.jar", SearchOption.AllDirectories);
                        namesOnly = filePaths.Select(f => Path.GetFileName(f)).OrderByDescending(x => x).ToArray();
                        main.apkToolComboBox.Items.Clear();
                        main.apkToolComboBox.Items.AddRange(namesOnly);
                        ...
                        main.LogOutput(Lang.Localize("update_apktool_lbl", "A new apktool version is now installed"), MainForm.Type.Info);
                    }
                    else
                        main.LogOutput(Lang.NO_UPDATE, MainForm.Type.Info);
        ...
        private void LoadConfig()
        {
            main.Invoke(new Action(delegate ()
            {
                ...
                try
                {
                    if (!main.disHisBox.Checked)
                    {
                        if (File.Exists(pathToHistory + "apkhistory.txt"))
                        {
                            string[] Lines = File.ReadAllLines(pathToHistory + "apkhistory.txt");

                            foreach (string Line in Lines)
                                main.pathOfApk.Items.Add(Line);
                        }

                        if (File.Exists(pathToHistory + "dechistory.txt"))
                        {
                            string[] Lines = File.ReadAllLines(pathToHistory + "dechistory.txt");

                            foreach (string Line in Lines)
                                main.decNameTextBox.Items.Add(Line);
                        }

                        if (File.Exists(pathToHistory + "comhistory.txt"))
                        {
                            string[] Lines = File.ReadAllLines(pathToHistory + "comhistory.txt");
        ...
        internal void SaveConfig()
        {
            try
            {
                ...
                if (!main.disHisBox.Checked)
                {
                    StreamWriter SaveFile = new StreamWriter(pathToHistory + "apkhistory.txt");
                    ...
                    StreamWriter SaveFile2 = new StreamWriter(pathToHistory + "dechistory.txt");
                    ...
                    StreamWriter SaveFile3 = new StreamWriter(pathToHistory + "comhistory.txt");
